// scripts/db/20250907_create_cps_profiles.mjs
// Self-contained: no imports from your app. Uses .env directly.
...
// ---- 0) Make sure we have DB env vars ----
...
// ---- 1) Create a direct Sequelize connection from env ----
...
async function run() {
  console.log(`✅ Connecting to PostgreSQL at ${process.env.DB_HOST}:${process.env.DB_PORT || 5432}...`);
  ...
  try {
    ...
    await sequelize.query(
      ...
      CREATE TABLE IF NOT EXISTS "user"."cps_profiles" (
        ...
        mean_response_time            NUMERIC(7,2) NOT NULL DEFAULT 0,
        ...
        decision_latency              NUMERIC(7,2) NOT NULL DEFAULT 0,
    ...
    // ---- 3) Backfill zero rows for users missing a cps profile ----
    await sequelize.query(
      `
